...
spark = SparkSession.builder.appName("PipelineSatClientes").config("spark.sql.extensions", "io.delta.sql.DeltasAdministradasparkSessionExtension").config("spark.sql.catalog.spark_catalog", "org.apache.spark.sql.delta.catalog.DeltaCatalog").config("fs.azure", "fs.azure.NativeAzureFileSystem").getOrCreate()
...
existe = spark.sql("""SELECT COUNT(*) AS existe FROM data_vault.information_schema.tables WHERE table_name = 'sat_clientes' AND table_schema = 'cri'""").first()["existe"]
...
existe = spark.sql("""SELECT COUNT(*) AS existe FROM data_vault.information_schema.tables WHERE table_name = 'sat_clientes_telefonos' AND table_schema = 'cri'""").first()["existe"]
...
existe = spark.sql("""SELECT COUNT(*) AS existe FROM data_vault.information_schema.tables WHERE table_name = 'sat_clientes_correos' AND table_schema = 'cri'""").first()["existe"]
...
# MAGIC %sql
# MAGIC INSERT INTO sat_clientes (hk_clientes, nombre_cliente, apellido_cliente, calle, ciudad, estado, codigo_zip, hk_diff, fecha_registro, nombre_fuente)
# MAGIC SELECT 
# MAGIC   A.hk_clientes
...
# MAGIC ;
# MAGIC
# MAGIC INSERT INTO sat_clientes_telefonos (hk_clientes, codigo_pais, numero_telefono, hk_diff, fecha_registro, nombre_fuente ) 
# MAGIC SELECT 
# MAGIC   A.hk_clientes
# MAGIC   ,A.codigo_pais
...
# MAGIC ;
# MAGIC
# MAGIC INSERT INTO sat_clientes_correos (hk_clientes, correo_electronico, hk_diff, fecha_registro, nombre_fuente )
# MAGIC SELECT 
# MAGIC   A.hk_clientes
