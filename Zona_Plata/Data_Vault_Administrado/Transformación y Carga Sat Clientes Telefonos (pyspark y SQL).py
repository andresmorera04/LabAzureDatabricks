...
if existe == 0: 
    ...
    spark.sql(f"""
          CREATE TABLE IF NOT EXISTS Sat_Cli_Clientes (
              Hk_Clientes BINARY NOT NULL COMMENT '',
              NombreCliente STRING NOT NULL COMMENT '',
              ApellidosCliente STRING NOT NULL COMMENT '',
              Calle STRING NOT NULL COMMENT '',
              Ciudad STRING NOT NULL COMMENT '',
              Estado STRING NOT NULL COMMENT '',
              CodigoZip STRING NOT NULL COMMENT '',
              HkDiff BINARY NOT NULL COMMENT '',
              Fecharegistro DATE NOT NULL COMMENT '',
              NombreFuente STRING NOT NULL COMMENT ''
          )
          USING DELTA 
...
existe = spark.sql("""SELECT COUNT(*) AS existe FROM data_vault.information_schema.tables WHERE table_name = 'Sat_Cli_Clientes_telefonos' AND table_schema = 'reg'""").first()["existe"]
...
existe = spark.sql("""SELECT COUNT(*) AS existe FROM data_vault.information_schema.tables WHERE table_name = 'Sat_Cli_Clientes_correos' AND table_schema = 'reg'""").first()["existe"]
...
# MAGIC %sql
# MAGIC INSERT INTO Sat_Cli_Clientes (hk_clientes, nombre_cliente, apellido_cliente, calle, ciudad, estado, codigo_zip, hk_diff, fecha_registro, nombre_fuente)
# MAGIC SELECT 
# MAGIC   A.hk_clientes
