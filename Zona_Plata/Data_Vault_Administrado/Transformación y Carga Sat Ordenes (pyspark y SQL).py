...
spark = SparkSession.builder.appName("PipelineSatOrdenes").config("spark.sql.extensions", "io.delta.sql.DeltasAdministradasparkSessionExtension").config("spark.sql.catalog.spark_catalog", "org.apache.spark.sql.delta.catalog.DeltaCatalog").config("fs.azure", "fs.azure.NativeAzureFileSystem").getOrCreate()
...
existe = spark.sql("""SELECT COUNT(*) AS existe FROM data_vault.information_schema.tables WHERE table_name = 'sat_ordenes_encabezado' AND table_schema = 'cri'""").first()["existe"]
...
existe = spark.sql("""SELECT COUNT(*) AS existe FROM data_vault.information_schema.tables WHERE table_name = 'sat_ordenes_detalle' AND table_schema = 'cri'""").first()["existe"]
...
resultado = spark.sql(f"""
                    INSERT INTO sat_ordenes_encabezado (hk_ordenes, estado_orden, fecha_orden, fecha_requerida, fecha_entrega, total, hk_diff, fecha_registro, nombre_fuente )
                    SELECT 
                    E.hk_ordernes
                    ,E.estado_orden
                    ,E.fecha_orden
...
resultado = spark.sql(f"""
                        INSERT INTO sat_ordenes_detalle (hk_ordenes, id_linea_detalle, id_producto, fecha_orden, cantidad, precio_lista, descuento, hk_diff, fecha_registro, nombre_fuente ) 
                        SELECT 
                        C.hk_ordenes
                        ,C.id_linea_detalle
                        ,C.id_producto 
