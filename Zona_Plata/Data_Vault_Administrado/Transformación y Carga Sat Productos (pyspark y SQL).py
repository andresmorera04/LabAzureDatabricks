...
spark = SparkSession.builder.appName("PipelineSatProductos").config("spark.sql.extensions", "io.delta.sql.DeltasAdministradasparkSessionExtension").config("spark.sql.catalog.spark_catalog", "org.apache.spark.sql.delta.catalog.DeltaCatalog").config("fs.azure", "fs.azure.NativeAzureFileSystem").getOrCreate()
...
existe = spark.sql("""SELECT COUNT(*) AS existe FROM data_vault.information_schema.tables WHERE table_name = 'sat_productos' AND table_schema = 'cri'""").first()["existe"]
...
# MAGIC (hk_productos, nombre, categoria, marca, modelo, precio_unitario, hk_diff, fecha_registro, nombre_fuente )
# MAGIC SELECT 
