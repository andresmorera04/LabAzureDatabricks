...
spark = SparkSession.builder.appName("PipelineDimProducto").config("spark.sql.extensions", "io.delta.sql.DeltasAdministradasparkSessionExtension").config("spark.sql.catalog.spark_catalog", "org.apache.spark.sql.delta.catalog.DeltaCatalog").config("fs.azure", "fs.azure.NativeAzureFileSystem").getOrCreate()
...
existeTabla = spark.sql("""SELECT COUNT(*) AS existe FROM dwh.information_schema.tables WHERE table_name = 'dim_producto' AND table_schema = 'cri'""").first()["existe"]
...
dfDatosNuevosExistentes = spark.sql("""
                                    ...
                                    SELECT 
                                        ...
                                        ,B.precio_unitario
                                    FROM 
                                        data_vault.cri.hub_productos AS A 
                                        INNER JOIN 
                                        (
                                        SELECT 
                                            B2.hk_productos
                                        ...
                                        (
                                            ...
                                            ,modelo
                                            ,precio_unitario
                                            ,ROW_NUMBER()OVER(PARTITION BY hk_productos ORDER BY fecha_registro DESC) AS desduplicador
                                            FROM 
                                            data_vault.cri.sat_productos 
                                        ) AS B2
                                        WHERE 
                                            B2.desduplicador = 1
                                        ) AS B
                                    ...
                                    INNER JOIN dwh.cri.dim_producto AS D 
...
dfTempNuevos = spark.sql(f"""
                        ...
                        LEFT JOIN dwh.cri.dim_producto AS D 
                            ON C.bk_id_producto = D.llave_negocio_producto
                        WHERE
...
if dfDatosNuevosExistentes.isEmpty() == False : 
    dfDimCliente = DeltaTable.forName(spark, "dwh.cri.dim_producto")
