...
spark = SparkSession.builder.appName("PipelineDimTiempo").config("spark.sql.extensions", "io.delta.sql.DeltasAdministradasparkSessionExtension").config("spark.sql.catalog.spark_catalog", "org.apache.spark.sql.delta.catalog.DeltaCatalog").config("fs.azure", "fs.azure.NativeAzureFileSystem").getOrCreate()
...
existeTabla = spark.sql("""SELECT COUNT(*) AS existe FROM dwh.information_schema.tables WHERE table_name = 'dim_tiempo' AND table_schema = 'cri'""").first()["existe"]
