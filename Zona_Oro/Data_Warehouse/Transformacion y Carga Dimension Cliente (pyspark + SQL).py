...
spark = SparkSession.builder.appName("PipelineDimCliente").config("spark.sql.extensions", "io.delta.sql.DeltaSparkSessionExtension").config("spark.sql.catalog.spark_catalog", "org.apache.spark.sql.delta.catalog.DeltaCatalog").config("fs.azure", "fs.azure.NativeAzureFileSystem").getOrCreate()
...
spark.sql(f"""
          ...
          USING DELTA
          LOCATION 'abfss://{contenedorDatalake}@{cuentaDatalake}.dfs.core.windows.net/Gold/Deltas/DWH/common/dim_cliente'
          """)
...
dfDatosNuevosExistentes = spark.sql("""
                                    SELECT 
                                        ...
                                        SELECT 
                                            ...
                                            ,B.codigo_zip
                                        FROM 
                                            data_vault.common.hub_clientes AS A 
                                            INNER JOIN 
                                            (
                                            SELECT 
                                                B2.hk_clientes
                                            ...
                                            FROM 
                                                ...
                                                FROM 
                                                    data_vault.common.sat_clientes 
                                                ) AS B2
                                            WHERE 
                                                B2.desduplicador = 1
                                            ) AS B 
                                            ...
                                            FROM 
                                                ...
                                                ,ROW_NUMBER()OVER(PARTITION BY hk_clientes ORDER BY fecha_registro DESC) AS desduplicador
                                                FROM 
                                                data_vault.common.sat_clientes_correos 
                                                ) AS C2
                                            WHERE 
                                                C2.desduplicador = 1
                                            ) AS C 
                                            ...
                                            FROM 
                                                ...
                                                ,ROW_NUMBER()OVER(PARTITION BY hk_clientes ORDER BY fecha_registro DESC) AS desduplicador
                                                FROM 
                                                data_vault.common.sat_clientes_telefonos 
                                                ) AS D2 
                                            WHERE 
                                                D2.desduplicador = 1
                                            ) AS D 
                                        ...
                                        INNER JOIN dwh.common.dim_cliente AS dim 
...
dfTempNuevos = spark.sql(f"""
                         SELECT 
                            ...
                            LEFT JOIN dwh.common.dim_cliente AS dim 
                                ON plata.bk_id_cliente = dim.llave_negocio_cliente
                            WHERE 
...
if dfDatosNuevosExistentes.isEmpty() == False : 
    dfDimCliente = DeltaTable.forName(spark, "dwh.common.dim_cliente")
...
# MAGIC %sql
# MAGIC SELECT * FROM dwh.common.dim_cliente;
